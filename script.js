import { cE, qS, qSA, createLogin, createCartModal, createProduct, addListenersToProducts } from "./utils/fn.js";
...
const categoryMenu = qS(".filters__category");
const searchInput = qS(".searchInput");
export let cartItems = [];
export let productListItems = [];
...
/* Listener per gestire il filtraggio per categoria dei prodotti */
categoryMenu.addEventListener("change", (event) => {
    if (event.target.value == "category") {
        productList.textContent = "";
        productListItems.forEach((product) => productList.append(createProduct(product)));
        ...
        productListItems.filter((product) =>
            ...
            .forEach((object) => {
                productList.append(createProduct(object));
            })
        addListenersToProducts();
    }
})
...
/* Listener per gestire la comparsa della modale del carrello */
