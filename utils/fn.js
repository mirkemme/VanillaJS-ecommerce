...
import { navbar, hero, filters, rootEl, cartItems, cartBtn, productListItems, productList, productListTitle } from "../script.js"
...
export const createCartModal = () => {
    ...
    items.className = "cartModal__items";
    closeBtn.className = "closeModalBtn";
    closeBtn.textContent = "X";
    ...
    cartItems.forEach((element) => {
        ...
        const title = cE("h4");
        const price = cE("h5");
        item.className = "cartModal__item";
    ...
    closeBtn.addEventListener("click", () => {
        ...
        // cartModal.classList.remove("cartModal__show");
...
/* Funzione che aggiunge il listener ad ogni card dei prodotti per la visualizzazione della modale della card scelta. */
export const addListenersToProducts = () => {
    const productCardEls = qSA(".productCard");
    productCardEls.forEach((element) =>
        element.addEventListener("click", () => {
            rootEl.append(createProductModal(productListItems.find((item) => item.id === parseInt(element.id)), rootEl));
        }
        )
    )
}